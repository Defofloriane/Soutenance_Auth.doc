...
$sum = array_sum(str_split($birthDayDigits));
   ...
   public function show(Request $request)
   {
      ...
      if($type=='releve'){
         ...
         $encryptedData = $hmac.'?'. $matricule .'?'. $niveau.'?'. $type;
   ...
   public function view_attestation_phone(Request $request)
   {
      ...
      if($type=='releve'){
         $releve = Releve::where(['id_releve'=>$id_releve,'etudiant'=>$matricule,'niveau'=>$niveau])->first();
         $donnees= Releve::where(['id_releve'=>$id_releve,'etudiant'=>$matricule,'niveau'=>$niveau])->first();
         $dataCont= 'releve'.'?'.trim($donnees->id_releve).'?'.trim($donnees->etudiant).'?'.trim($donnees->decision).'?'.trim($donnees->filiere).'?'.trim($donnees->niveau).'?'.trim((float)$donnees->mgp).'?'.trim($donnees->anneeAcademique);
       ...
       return view("details", compact('releve', 'etudiant', 'notes','hmacInfo'));
        ...
        //fin hachage et crypthage des informations
         ...
         // Passez les données à la vue de détails
         return view("view_attestation_phone", compact('releve', 'etudiant', 'hmacInfo'));
